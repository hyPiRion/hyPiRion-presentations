...
(defn gen-code [level]
  (let [code (gjs/htmlized-class level)
        parent (d/by-class "javacode")]
    (info (list 'gen-code level))
    (d/destroy-children! parent)
    (d/append! parent code)))
...
(def slideshow-keymap
  {goog.events.KeyCodes.T :toggle-mode
   ...
   goog.events.KeyCodes.THREE [:gen 3]
   goog.events.KeyCodes.SIX [:gen 6]
   goog.events.KeyCodes.NINE [:gen 9]})
...
(defn install-event-handlers []
  ...
  (dispatch/react-to #{[:gen 3]} (fn [id _] (gen-code 3)))
  (dispatch/react-to #{[:gen 6]} (fn [id _] (gen-code 6)))
  (dispatch/react-to #{[:gen 9]} (fn [id _] (gen-code 9))))
